...
- **自动划重点**:回答中的关键术语、结论与数字由模型以 **加粗** 标出(唯一允许的 Markdown 记号),前端安全渲染,不做 HTML 注入。
...
- **内置图解工具**:agent 判断流程、结构或层级关系难以用文字讲清时,自主调用 `draw_diagram` 生成 mermaid 流程图/思维导图(每答最多 3 张,内容由检索证据约束),随回答一起展示与保存。
...
5. `railway up` 打包上传上下文时遵循 git 的可见性规则,内置教材因此需要保持「不进 Git、但进构建上下文」:它们是普通的未跟踪文件,并靠本地 pre-commit 钩子防止误提交。重新克隆后恢复防线:
